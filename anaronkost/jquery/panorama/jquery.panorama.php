	...
	$.fn.panorama = function(options) {
		this.each(function(){ 
			...
			panoramaContainer.css('height', elemHeight+'px').css('overflow', 'hidden').wrap("<div class='panorama-viewport'></div>").parent().css('width',settings.viewport_width+'px')
				.append("<div class='panorama-control'><a href='#' class='panorama-control-left'><<</a> <a href='#' class='panorama-control-pause'>x</a> <a href='#' class='panorama-control-right'>>></a><ul href='#' class='nav-map'><li><?if($bahasa ==1) echo 'Peta'; else echo 'Maps';?><ul><?
				$o=0;
				$queryselect = mysql_query("SELECT * FROM lantaigedung ORDER BY id_gedung ASC");
				while($select = mysql_fetch_array($queryselect)){
				$idgedung=$select['id_gedung'];
				$qselect1=mysql_query("SELECT * FROM gedung WHERE id_gedung = '$idgedung'");
				$s1 = mysql_fetch_array($qselect1);
				?><a href='#' class='peta<?=$o?>'><li><?if($bahasa==1) echo $s1['judul_ina'].' '.$select['judul_ina']; else echo $s1['judul_eng'].' '.$select['judul_eng'];?></li></a><?$o++;}?></ul></li></ul></div>");
			...
			//test
			<?for($p=0;$p<=$o;$p++){?>
			panoramaViewport.find('a.peta<?=$p?>').bind('click', function() {
				$.blockUI({ message: $('#mappopup<?=$p?>'), css: { width: '275px', left:'15%', top:'3%' } }); 
				$('.blockOverlay').attr('title','<?if($bahasa==1) echo 'Klik untuk kembali'; else echo 'Click to unblock';?>').click($.unblockUI);
			});
			<?}?>
